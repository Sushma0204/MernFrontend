...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      const res = await axios.get("https://mernbackend-wf2s.onrender.com/api/users/" + user._id)
  ...
  const handleUserUpdate = async () => {
    setUpdated(false)
    try {
      const res = await axios.put("https://mernbackend-wf2s.onrender.com/api/users/" + user._id, { username, email, password }, { withCredentials: true })
  ...
  const handleUserDelete = async () => {
    try {
      const res = await axios.delete("https://mernbackend-wf2s.onrender.com/api/users/" + user._id, { withCredentials: true })
      setUser(null)
      navigate("/")
      // console.log(res.data)
  ...
  const fetchUserPosts = async () => {
    try {
      const res = await axios.get("https://mernbackend-wf2s.onrender.com/api/posts/user/" + user._id)
      // console.log(res.data)
      setPosts(res.data)
